...
X = np.loadtxt('../data/ckc2d_5_samples_noJL.db')
# X = X[:int(1e5),:]
n_test = 10000
n = X.shape[0]-n_test
...
x_train = normz(X[0:n,5:], x_max, x_min)
x_test = normz(X[n:,5:], x_max, x_min)
...
# Network Parameters
num_encoded = 2
hidden_layers = [10]*5#[3,3,3,3,3,3,3,3, num_encoded]
...
decoder_direct_op = decoder(Z, weights, biases)
...
with tf.Session() as sess:
    ...
    if training:
        ...
        if  not retrain:
            ...
            saver.restore(sess, "./models/model_5bars_3.ckpt")                
            print("Loaded saved model: %s" % "./models/model_5bars_3.ckpt")
        ...
        save_path = saver.save(sess, "./models/model_5bars_3.ckpt")
...
plt.figure(3)
plt.plot(z_train_pred[:,0], z_train_pred[:,1],'bo')
plt.plot(z_test_pred[:,0], z_test_pred[:,1],'go')
